
"""
AES-128 (CBC mode) implemented from scratch.
No cryptography libraries used — pure Python.

Reference: FIPS-197 (NIST AES standard)
"""

import os

# ─────────────────────────────────────────────
# AES S-box (SubBytes lookup table)
# ─────────────────────────────────────────────
SBOX = [
    ,
]

# Inverse S-box, derived from SBOX (not hand-transcribed — safer, no typo risk)
INV_SBOX = [0] * 256
for i, val in enumerate(SBOX):
    INV_SBOX[val] = i

# Round constants used in key expansion
RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]

BLOCK_SIZE = 16   # AES operates on 16-byte (128-bit) blocks
KEY_SIZE = 16     # AES-128 → 16-byte (128-bit) key
NUM_ROUNDS = 10   # AES-128 uses 10 rounds


# ─────────────────────────────────────────────
# Galois Field GF(2^8) multiplication
# Needed for MixColumns / InvMixColumns
# ─────────────────────────────────────────────
def xtime(a):
    """Multiply by 2 in GF(2^8)"""
    a <<= 1
    if a & 0x100:
        a ^= 0x11B  # reduce by AES's irreducible polynomial x^8+x^4+x^3+x+1
    return a & 0xFF


def gmul(a, b):
    """General multiplication in GF(2^8)"""
    result = 0
    for _ in range(8):
        if b & 1:
            result ^= a
        a = xtime(a)
        b >>= 1
    return result & 0xFF


# ─────────────────────────────────────────────
# Key expansion — turns the 16-byte key into
# 11 round keys (44 words of 4 bytes each)
# ─────────────────────────────────────────────
def key_expansion(key: bytes):
    key_symbols = list(key)
    w = [list(key_symbols[4 * i:4 * i + 4]) for i in range(4)]  # first 4 words = original key

    for i in range(4, 4 * (NUM_ROUNDS + 1)):
        temp = list(w[i - 1])
        if i % 4 == 0:
            # RotWord
            temp = temp[1:] + temp[:1]
            # SubWord
            temp = [SBOX[b] for b in temp]
            # XOR with Rcon
            temp[0] ^= RCON[i // 4 - 1]
        w.append([w[i - 4][j] ^ temp[j] for j in range(4)])

    return w  # list of 44 words


def words_to_round_key(w, round_num):
    """Extract round key (4 words) as a 4x4 state matrix (column-major)"""
    words = w[4 * round_num: 4 * round_num + 4]
    return [[words[c][r] for c in range(4)] for r in range(4)]


# ─────────────────────────────────────────────
# State transformations
# ─────────────────────────────────────────────
def bytes_to_state(block: bytes):
    """16 bytes -> 4x4 column-major state matrix"""
    return [[block[r + 4 * c] for c in range(4)] for r in range(4)]


def state_to_bytes(state):
    return bytes(state[r][c] for c in range(4) for r in range(4))


def add_round_key(state, round_key):
    return [[state[r][c] ^ round_key[r][c] for c in range(4)] for r in range(4)]


def sub_bytes(state):
    return [[SBOX[state[r][c]] for c in range(4)] for r in range(4)]


def inv_sub_bytes(state):
    return [[INV_SBOX[state[r][c]] for c in range(4)] for r in range(4)]


def shift_rows(state):
    new_state = [row[:] for row in state]
    for r in range(1, 4):
        new_state[r] = state[r][r:] + state[r][:r]
    return new_state


def inv_shift_rows(state):
    new_state = [row[:] for row in state]
    for r in range(1, 4):
        new_state[r] = state[r][-r:] + state[r][:-r]
    return new_state


def mix_columns(state):
    new_state = [[0] * 4 for _ in range(4)]
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        new_state[0][c] = gmul(col[0], 2) ^ gmul(col[1], 3) ^ col[2] ^ col[3]
        new_state[1][c] = col[0] ^ gmul(col[1], 2) ^ gmul(col[2], 3) ^ col[3]
        new_state[2][c] = col[0] ^ col[1] ^ gmul(col[2], 2) ^ gmul(col[3], 3)
        new_state[3][c] = gmul(col[0], 3) ^ col[1] ^ col[2] ^ gmul(col[3], 2)
    return new_state


def inv_mix_columns(state):
    new_state = [[0] * 4 for _ in range(4)]
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        new_state[0][c] = gmul(col[0], 14) ^ gmul(col[1], 11) ^ gmul(col[2], 13) ^ gmul(col[3], 9)
        new_state[1][c] = gmul(col[0], 9) ^ gmul(col[1], 14) ^ gmul(col[2], 11) ^ gmul(col[3], 13)
        new_state[2][c] = gmul(col[0], 13) ^ gmul(col[1], 9) ^ gmul(col[2], 14) ^ gmul(col[3], 11)
        new_state[3][c] = gmul(col[0], 11) ^ gmul(col[1], 13) ^ gmul(col[2], 9) ^ gmul(col[3], 14)
    return new_state


# ─────────────────────────────────────────────
# Single 16-byte block encrypt / decrypt
# ─────────────────────────────────────────────
def encrypt_block(block: bytes, key: bytes) -> bytes:
    w = key_expansion(key)
    state = bytes_to_state(block)

    state = add_round_key(state, words_to_round_key(w, 0))

    for round_num in range(1, NUM_ROUNDS):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, words_to_round_key(w, round_num))

    # Final round — no MixColumns
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, words_to_round_key(w, NUM_ROUNDS))

    return state_to_bytes(state)


def decrypt_block(block: bytes, key: bytes) -> bytes:
    w = key_expansion(key)
    state = bytes_to_state(block)

    state = add_round_key(state, words_to_round_key(w, NUM_ROUNDS))

    for round_num in range(NUM_ROUNDS - 1, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, words_to_round_key(w, round_num))
        state = inv_mix_columns(state)

    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, words_to_round_key(w, 0))

    return state_to_bytes(state)


# ─────────────────────────────────────────────
# PKCS7 padding — AES needs full 16-byte blocks
# ─────────────────────────────────────────────
def pkcs7_pad(data: bytes) -> bytes:
    pad_len = BLOCK_SIZE - (len(data) % BLOCK_SIZE)
    return data + bytes([pad_len] * pad_len)


def pkcs7_unpad(data: bytes) -> bytes:
    pad_len = data[-1]
    if pad_len < 1 or pad_len > BLOCK_SIZE:
        raise ValueError("Invalid padding")
    return data[:-pad_len]


# ─────────────────────────────────────────────
# Public functions — CBC mode
# This is what records/views.py will actually call
# ─────────────────────────────────────────────
def aes_encrypt(plaintext: bytes, key: bytes) -> bytes:
    """
    Encrypts plaintext using AES-128 CBC.
    Returns: IV (16 bytes) + ciphertext, concatenated.
    The IV must travel with the ciphertext — it's not secret,
    just needs to be unique per encryption.
    """
    if len(key) != KEY_SIZE:
        raise ValueError("Key must be 16 bytes for AES-128")

    iv = os.urandom(BLOCK_SIZE)
    padded = pkcs7_pad(plaintext)

    ciphertext = b""
    prev_block = iv
    for i in range(0, len(padded), BLOCK_SIZE):
        block = padded[i:i + BLOCK_SIZE]
        xored = bytes(a ^ b for a, b in zip(block, prev_block))
        encrypted = encrypt_block(xored, key)
        ciphertext += encrypted
        prev_block = encrypted

    return iv + ciphertext


def aes_decrypt(data: bytes, key: bytes) -> bytes:
    """
    data = IV (first 16 bytes) + ciphertext
    Returns original plaintext.
    """
    if len(key) != KEY_SIZE:
        raise ValueError("Key must be 16 bytes for AES-128")

    iv = data[:BLOCK_SIZE]
    ciphertext = data[BLOCK_SIZE:]

    plaintext = b""
    prev_block = iv
    for i in range(0, len(ciphertext), BLOCK_SIZE):
        block = ciphertext[i:i + BLOCK_SIZE]
        decrypted = decrypt_block(block, key)
        xored = bytes(a ^ b for a, b in zip(decrypted, prev_block))
        plaintext += xored
        prev_block = block

    return pkcs7_unpad(plaintext)


# ─────────────────────────────────────────────
# Self-test using the OFFICIAL FIPS-197 test vector
# Run: python crypto/aes.py
# ─────────────────────────────────────────────
if __name__ == "__main__":
    test_key = bytes.fromhex("000102030405060708090a0b0c0d0e0f")
    test_plain = bytes.fromhex("00112233445566778899aabbccddeeff")
    expected_cipher = bytes.fromhex("69c4e0d86a7b0430d8cdb78070b4c55a")

    result = encrypt_block(test_plain, test_key)
    print("Single block encrypt test:")
    print("Got:      ", result.hex())
    print("Expected: ", expected_cipher.hex())
    assert result == expected_cipher, "AES block encryption is WRONG — do not proceed"
    print("✅ Block cipher matches official FIPS-197 test vector\n")

    decrypted = decrypt_block(result, test_key)
    assert decrypted == test_plain, "AES block decryption is WRONG"
    print("✅ Block decryption reverses correctly\n")

    # Full CBC round-trip test with real-length data
    my_key = os.urandom(16)
    message = b"Patient has penicillin allergy, blood group O+"
    encrypted = aes_encrypt(message, my_key)
    decrypted = aes_decrypt(encrypted, my_key)
    assert decrypted == message
    print("✅ CBC encrypt/decrypt round-trip works")
    print("Original: ", message)
    print("Decrypted:", decrypted)